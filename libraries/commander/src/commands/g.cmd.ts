...
const G_TYPE = ['vue' as const, 'next' as const, 'react' as const];

export interface GCommandOption {
  manager: 'npm' | 'pnpm' | 'yarn';
}

function createNextProject(option: GCommandOption) {
  const loading = ora();
  loading.start(Logger.info('正在通过 create-next-app@latest 创建模板', false));
  try {
    switch (option.manager) {
      case 'pnpm':
        execSync('pnpm dlx create-next-app@latest', { stdio: 'inherit' });
        break;
      case 'yarn':
      case 'npm':
      default:
