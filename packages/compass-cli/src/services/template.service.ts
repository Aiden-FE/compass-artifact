...
export default class TemplateService {
  ...
  static async pull(collectInfo: CollectInfoOptions) {
    ...
    await this.pullRepo(
      this.generateRepoUrl(collectInfo.target),
      collectInfo.replaceData.name,
      collectInfo.commandOptions.clone,
    );
    loading.succeed(LoggerService.success('模板拉取成功', false));
    await this.replaceFilesVars(
      this.generateReplaceFilesPath(
        collectInfo.target,
        collectInfo.replaceData.name,
      ),
      collectInfo.replaceData,
    );
  ...
  private static generateReplaceFilesPath(target: CollectInfoOptions['target'], name: string) {
    ...
    return (TEMPLATE_REPLACE_FILES_PATH[target] || [])
      .map((filePath) => path.join(basePath, name, filePath));
  }
